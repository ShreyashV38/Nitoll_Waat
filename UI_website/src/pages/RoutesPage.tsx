...
const RoutesPage = () => {
    ...
    const handleDispatch = async () => {
        if (!selectedDriver || !selectedWard) return showAlert({ title: "Validation Error", message: "Please select a Driver and a Ward", danger: true });
        ...
        try {
            ...
            showAlert({ title: "Success", message: "Driver Dispatched Successfully!" });
            setSelectedDriver("");
            setSelectedWard("");
            loadData();

        } catch (err: any) {
            showAlert({ title: "Error", message: err.response?.data?.message || "Failed to dispatch driver", danger: true });
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleAutoDispatch = async () => {
        ...
        try {
            ...
            showAlert({ title: "Operations Started", message: "Daily operations started! Drivers dispatched to assigned wards." });
            loadData();
        } catch (err: any) {
            showAlert({ title: "Dispatch Failed", message: err.response?.data?.message || "Auto-dispatch failed", danger: true });
        } finally {
    ...
    const handleCancelRoute = async (routeId: string) => {
        const isConfirmed = await showConfirm({
            title: "Remove Driver?",
            message: "Are you sure you want to remove this driver from the route? This action cannot be undone.",
        ...
        try {
            ...
            showAlert({ title: "Route Removed", message: "Driver is now available for reassignment." });
            loadData();
        } catch (err: any) {
            showAlert({ title: "Error", message: err.response?.data?.message || "Failed to remove route", danger: true });
        }
    };
